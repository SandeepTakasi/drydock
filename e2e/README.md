...
**GENERATED BY SEATRIAL, AND NOW EXECUTED IN CI.** `@playwright/test` is still
not a dependency of this repo and seatrial is still forbidden from adding it —
that constraint is about the repo's dependency list, not about whether anyone may
run the specs. `.github/workflows/verify.yml` installs Playwright into a scratch
directory outside the tree and runs all six on every push.

**First executed 2026-09-02, and they pass — 6 passed (8.7s).** TG2 and TG3 are
`test.fail()` inversions, so "passing" means they failed exactly as designed.
Until that date this file correctly said *"Nothing here has been run by a test
runner"*: the specs were a hypothesis about repeatability. They are now a
measured one, and the measurement is stronger than expected — generated at commit
`5a32ac9` and still green against an export many releases later.

What that does **not** establish: one browser (Chromium), one viewport, one
target. The caveats below stand unchanged.
