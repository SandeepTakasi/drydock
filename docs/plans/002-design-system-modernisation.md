...
| 8 | — (verified by T1.R.1, orchestrator-confirmed) | **`Variants` is an index-signature type, so a misspelled state name typechecks.** `{ hidden: {…}, shwon: {…} }` compiles clean under this repo's own tsconfig — reproduced independently. The plan's rule "state names stay exactly `hidden`/`shown` (silent failure otherwise)" is therefore **unenforceable by the type system and unchecked by any gate** | `Variants` from `motion/react` accepts arbitrary string keys by design | **Inherited, not introduced — but plan 004 adds six more call sites to it.** Ninth instance of this repo's "mistake that produces no error" class. Fix: whoever next owns `lib/motion.ts` should type the state names as a literal union so a typo becomes a compile error. Until then it is a docblock convention only |
| 9 | — (verified by T1.R.1, orchestrator-confirmed) | **`out/index.html` is not deterministic across builds of identical source.** Two consecutive builds on an unchanged tree produced different files, differing in a random build-id token (`IE9cdZlYEUDQxGxIJvPp0` vs `5eOl8oeYtWMNIV7a0zmfx`). The compiled CSS chunk **is** stable | Next embeds a per-build id in the RSC payload | **A trap for plan 005:** any criterion that checksums `index.html` will fail on every build regardless of source changes. This plan's gate happens to target `out/_next/static/chunks/*.css`, which is stable — correct by luck as much as design. T1.R.1's stronger suggestion: compare the emitted **file-name set** (45 files, content-hashed names) rather than one file's bytes |
| 10 | — (T1.R.1 finding, accepted) | `drawLine` and `revealClip` have had **zero consumers across two plans** and should be deleted. The three exports plan 002 added are **not** in that category: `assert-copy.mjs` hard-fails a section file containing `duration:`/`delay:`, so plan 003 literally cannot author a hero beat until the cadence is exported — the gate enforces contract-first | Earlier speculative generality in `lib/motion.ts` | Position taken rather than left open: contract-first sequencing justifies unconsumed exports **when a gate makes the alternative impossible**, and does not justify the two older ones. Deletion assigned to whoever next owns the file |
