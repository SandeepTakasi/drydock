...
|---|---|---|---|---|---|
| 15 | T2.1.1 / T2.1.2 | **Wave 2.1 declares two tasks that cannot run in parallel: `T2.1.2` **Depends on:** `T2.1.1`, and both sit under `### Wave 2.1 — Run the gate`.** Spec generation consumes the verdict sheet its sibling produces, so the pair is inherently sequential; the wave was declared parallel and executed in order (`08b6556` then `d79aa83`, both 2026-08-20 — see deviation 12, which recorded the commits landing out of order while "the work ran in order"). Found retroactively on 2026-09-02 by `drydock-audit.mjs validate-plan --strict`, which did not exist when this plan was written or executed. | The prohibition exists because simultaneous tasks cannot depend on each other, and at the time this plan was authored a plan had no way to say it runs its waves sequentially — `execution:` was not added until v0.8.0. Absent the key, every plan is read as `fleet`, so a sequential pair had nowhere to be declared except as a same-wave dependency. | **ACCEPTED, not repaired.** No wrong output: the tasks ran in the order the dependency states, both acceptance criteria passed independently, and both diffs stayed inside their owned files. Repairing it would mean editing a closed record — completed waves are immutable and ids are never reused — to satisfy a validator written after the fact, which would make the plan describe an execution that did not happen. **What it is evidence for:** this is the exact shape `execution: solo` was invented for in v0.8.0 (plan 005), so the rule and the escape hatch both exist now; a plan written today would declare `execution: solo` and validate clean. The plan therefore stays a permanent `--strict` failure on this one line, deliberately, as the record of why that key was added. | `validate-plan --strict`, retroactively 2026-09-02 |
