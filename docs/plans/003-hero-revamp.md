...
| 2026-08-19 | — (post-close addendum, deviation 12) | **The harness's error path was measured, not assumed: it exits 1.** | Recorded because A3's published claim rests on "both gates exit 0", which is worthless if the failure path also exits 0. Forced by holding both candidate ports (9222 was already held by the everyday Chrome, 9333 bound by a dummy listener): `measure-reduced-motion: ERROR — Chrome never came up on any of ports 9222, 9333` with **real exit 1**. A first attempt appeared to show exit 0, which was an artifact of piping to `tail` — `$?` was `tail`'s status, not the harness's. **Deviation 12 stands and is now bounded:** all-ports-busy fails loudly, so the unsound case is narrower than feared — a *stranger answering CDP* on a candidate port, not a busy port as such. Also note the harness needs 9333 free in practice, since a normal Chrome session holds 9222. |
