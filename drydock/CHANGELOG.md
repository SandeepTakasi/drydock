...
73 more in the strings the tools print, repunctuated one at a time rather than
swapped for another dash: a colon where a lead-in introduces a list, a semicolon
or a full stop where an independent clause follows, parentheses where an aside
was being set off, a comma only where a genuine appositive sits. There were no
prose double-hyphens to remove; the three in the plugin are git's `--` pathspec
separator and are untouched.

**Three things had to be handled as format rather than punctuation.**

- **The wavecheck report heading is parsed.** `WAVECHECK_RE` and the
  sealed-record verdict both matched on the em dash in
  `### Wavecheck 1.0 - PASS - <date>`, and all five plans in this repo were
  written with one. Wavecheck now emits a hyphen and both regexes accept either,
  so old plans keep parsing. Caught by running `plan-status` over the corpus,
  not by reading.
- **`N/A - <reason>`** in the Testing Gate is checked by
  `/N\/A\s*[—-]\s*\S/`, which accepts a dash and rejects a comma. Three skills
  had been rewritten to `N/A, <reason>`, which the validator would have refused.
- **The plan template's task and wave headings** now use a hyphen. Verified first
  that nothing parses the separator: task and wave headings match on the id, and
  `GATE_CLOSED` matches on "approved by" plus a date.

**Two bugs I introduced and caught, both worth recording.** Narrowing
`WAVECHECK_RE` to `[^—-]*` excluded hyphens, so "re-audit" split the heading
early and plan 004's four re-audit reports vanished; the fix is a non-greedy
`.*?`. And running the prose pass over the Node sources rewrote a **regex
character class**, `[—-]` becoming `[, -]`, which then matched the space before
"Decision" and read a verdict of "D". Both were caught by the suite rather than
by inspection, which is the argument for the suite.

Code comments and test fixtures keep their em dashes. Comments are not
user-facing, and the fixtures deliberately carry the old heading form, so they
now double as proof that the parser still reads plans written before this
release.
