...
It checks whether the configured `plans_dir` (default `docs/plans`) is gitignored, since some repos forbid committing tool or planning artifacts outright, which makes that path uncommittable, and falls back to `.drydock/plans/` when it is, alongside the other execution artifacts. Then **quote its `**Plan location:**` sentence verbatim** into the plan, directly under the title. Do not write your own justification for the location: that improvisation is the defect this replaces, because the reasoning came out different in every plan and a reader had to reconstruct why the file sat where it sat. One sentence, the same wording every time, generated rather than argued.
...
- [ ] **`node ${CLAUDE_PLUGIN_ROOT}/scripts/drydock-audit.mjs validate-plan --strict <plan-path>` exits 0.** This skill requires every task it writes to carry "one command that exits 0" and shipped none for its own output; this is that command. It checks what a reader cannot reliably eyeball across a thousand-line document: duplicate task ids, two tasks in one wave owning the same file, a dependency on a task in the same wave (those tasks are declared parallel and cannot be), required sections and their order, and Testing Gate completeness. Run it before the pressure test, a plan defect multiplies across every agent that executes it, and the first run of this check reported a same-wave dependency in two of this repo's plans that reviews, gates and a pressure test had all missed. One was real (accepted, and recorded as that plan's deviation 15); the other was the checker reading a task's wave off its id when the contract says an id outlives its wave assignment, and was fixed in v0.8.10. Both halves are worth knowing before you trust a report from it: it finds what a reader cannot, and it is not above being wrong.
