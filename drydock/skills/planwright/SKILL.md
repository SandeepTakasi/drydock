---
name: planwright
description: Craft a rigorous, surgical implementation plan with phases, parallel waves, atomic tasks, right-sized model assignments and verification gates, delivered as a persistent plan document, without writing any implementation code. Use this skill whenever the user asks to "create a plan", "plan this feature/change", "break this down", "design phases and tasks", wants a task breakdown for subagent or parallel execution, or requests any multi-file change large enough that a plan should precede implementation, even when they never say the word "plan".
---
...
**First check for a host profile.** If `drydock.config.yaml` exists at the repo root (or the path in userConfig `config_path`), read it and SKIP every interview question it already answers, naming it as the source in section 5 rather than re-deriving the value. A profile holds defaults, not facts: any plan may override one, and an override goes in the Decision Log with its reason, because a profile that silently shapes a plan it no longer describes is drift. Absent profile means interview as below, and say so once, in one line, rather than asking the user to run `drydock:init` mid-plan.

Then interview the user about the practices the plan must abide by; `reference/practices-interview.md` holds the full question bank. Establish: testing approach, review gates (human approval, manual confirmation, PR review), version-control conventions (branching, commit granularity, worktrees), quality gates (lint, typecheck, build, CI), execution preferences (subagent parallelism, model budget, worktree isolation, see step 4), whether a spec already exists, and whether tasks should be mirrored into a tracker. Batch these into **one** message using whatever selection UI is available, or a compact numbered list. Do not drip-feed questions across turns.

**If the change touches a UI or API surface, the interview must also produce testable acceptance criteria**: the base URL and the command that starts the app, the auth approach, evidence retention, and which outcomes are blockers. That is what makes the **Testing Gate** (step 6) writable at plan time instead of invented afterwards. The question block is in `reference/practices-interview.md` under *End-to-end verification*. "There is no browser-drivable surface here" is a complete answer: it becomes the section's `N/A, <reason>`.
...
**TDD does not mean a RED wave and a GREEN wave.** A wave boundary is a synchronisation point with a five-check gate attached, and red/green does not need one; splitting them turns every parallel wave into a sequential pair and buys nothing. Put the failing test and the code that satisfies it in **one task**, with the test passing as the acceptance criterion and the executor told to watch it fail first. Where the user genuinely wants them separate, that ordering is legal in a single wave only under `execution: solo`; under `fleet` the tasks are simultaneous, the dependency cannot hold, and `validate-plan` rejects it.

### Step 2: Explore the codebase

Map everything the change touches: entry points, affected modules, data models, migrations, contracts between services, existing tests, and the conventions already in use. If subagents are available, delegate exploration to them and keep only their conclusions, exploration noise (dozens of file reads and greps) must not pollute the planning context. If not, explore inline but summarize findings into a short constraints list before moving on.
...
- **Attribution (ask the host repo, not the default).** Write `attribution: manifest` unless the repo is happy with `drydock(<task-id>): ...` commit subjects. Under `manifest` each executor runs `drydock-audit.mjs task-close` after its checkpoint commit and the subject follows the host's own convention, so a repo whose policy forbids tool names or task ids in subjects can run the plan unmodified; that policy is otherwise a hard BLOCK on every wave. It is also the default from `format_version: 3`, because a commit subject is not unique: any later commit reusing a task id makes the wave unauditable. `commit-prefix` stays correct for repos with no such policy. Neither mode removes the per-task commit, since attribution from a combined diff is unsound.
- **Solo execution (ask, do not assume).** Step 1 asks whether `drydock:executor` subagents will actually be spawned. If they will not, write `execution: solo`. Two things follow. Same-wave dependencies become legal, because the prohibition exists for simultaneity and there is none. And the plan states **once** that the session writing the diff is the session auditing it, instead of asserting a fleet and logging the same Deviation 1 on every wave. `solo` relaxes no gate, no ownership boundary and no acceptance criterion; it removes a claim the plan was making falsely. Default is `fleet`.
...
**Staleness check on resume:** plans are written as if the repo freezes; on a real team it does not. Before any wave starts, diff the wave's owned files, and its wave-0 contract files, against the baseline SHA recorded by T0. Any change since planning makes the wave stale: re-validate its tasks against current code and update the baseline SHA and Decision Log. Never execute a stale wave on the original assumptions. If staleness invalidates task contents rather than just context, recommend `/drydock:replan` instead of patching ad hoc.
...
**Resolve where the plan goes; do not argue for it.** Run:

```
node ${CLAUDE_PLUGIN_ROOT}/scripts/drydock-audit.mjs resolve-plans-dir
```

It checks whether the configured `plans_dir` (default `docs/plans`) is gitignored, since some repos forbid committing tool artifacts, and falls back to `.drydock/plans/` when it is. Then **quote its `**Plan location:**` sentence verbatim** into the plan, under the title. Do not write your own justification for the location: improvised reasoning came out different in every plan and left a reader reconstructing why the file sat where it sat.

Write the plan as `NNN-<feature-slug>.md` in the directory it resolved, using the exact structure in `reference/plan-format.md`. If a plans directory already exists in the repo under another name, prefer it and pass it as the argument.

**Write the Testing Gate (§11) for any plan touching a UI or API surface.** Schema and gate rule are in `reference/plan-format.md` under *Testing Gate section*; `drydock:seatrial` executes it after the final wave and `drydock:reconcile` will not close the plan without a GO verdict. Author it **now**, from the interview's acceptance criteria: a gate written after implementation tests what was built rather than what was asked for. Each case needs an id, a title, preconditions, Given/When/Then steps performable exactly as written, an expected result, one declared evidence type, and a severity. **Do not declare an evidence type the driver cannot capture.** Confirm the capability this session and default to `screenshot` or `network assertion` otherwise; `video` is a per-`BrowserContext` setting fixed at creation, so a Playwright MCP server started without it cannot produce it by any call, and a case declaring it fails its evidence clause on every run however correct the application is. That red describes your harness, not the software. Where the capability is genuinely unknown at plan time, say so in the case and flag it for a human. Include at least one case designed to FAIL, and say so in its expected result: a gate nobody has watched fail is not known to work. With no browser-drivable surface the whole section is `N/A, <reason>`; the reason is required and "not applicable" is not one. Embed the Orchestrator contract from that file verbatim in the preamble. The plan is machine-readable by design, so a separate session can consume it and check off progress. Chat output is not a deliverable; the file is.
...
- [ ] **Every acceptance criterion was RUN, against the real file and the real shell, before the task block was frozen.** Ask both halves: can it fail, *and can it pass?* A criterion authored from memory of a platform's output or a file's shape is the most common way a task ships an inert or unpassable gate, and Mechanical- and Standard-tier criteria, which get the least scrutiny, break this way most. Confirm each literal pattern against the convention it greps: heading level, exact output, padding. Three criteria in one plan failed this way: one unpassable regardless of repo state (BSD `wc` left-pads, so `grep -qx 1` could never match), one already satisfied before its task began, one grepping `####` at a file that uses `##` throughout. Two of those three failed loudly at the gate; the one that was **already satisfied** is the dangerous shape, because the task could do nothing and still be marked done. `node ${CLAUDE_PLUGIN_ROOT}/scripts/drydock-audit.mjs prove-failable <plan-path>` runs every command-shaped criterion in the current tree and errors on any that already exits 0. Run it before the plan is approved, while the tree is still the baseline.
- [ ] **§11 Testing Gate is written, or is `N/A` with a real reason.** If the plan touches a UI or API surface: every case has all seven fields, every declared evidence type is `screenshot` or `network assertion` **and the connected driver has been confirmed this session to capture it** (`video` is rejected outright, see below), at least one case is designed to fail and declares that inversion, and the target's base URL and start command came from the user rather than from a guess. An absent section on a UI-touching plan, or a bare "N/A", fails this item.
- [ ] **`node ${CLAUDE_PLUGIN_ROOT}/scripts/drydock-audit.mjs validate-plan --strict <plan-path>` exits 0.** This skill demands every task carry "one command that exits 0"; this is that command for its own output. It checks what a reader cannot eyeball across a thousand-line document: duplicate task ids, two tasks in one wave owning the same file, a dependency on a task in the same wave, required sections and their order, Testing Gate completeness, and a plan the parser reads as having no tasks at all. Run it before the pressure test. It is also not above being wrong: its first run reported a same-wave dependency in two plans here, one real and accepted, one its own defect. Trust it to find what a reader cannot, not to be infallible.
...
**Adversarial pressure-test.** After the checklist passes, dispatch one fresh-context reviewer agent (Judgment tier) whose only job is to break the plan before the user sees it: verify every claimed file path and interface exists, hunt for hidden dependencies between tasks marked parallel, confirm **every acceptance criterion fails at baseline** (`prove-failable` answers the command-shaped ones mechanically; the reviewer still has to judge the prose ones), and challenge each task with "what does the executing agent not know that will make it guess?" You wrote the plan, so you have the same author-blindness toward it that code authors have toward their code, and a plan defect multiplies across every agent that executes it. Fix confirmed findings, log the material ones in the Decision Log, and record the verdict in the plan. If subagents are unavailable, do the pass yourself with explicit fresh eyes: re-open the actual files and verify claims against the repo, not against memory.
