...
It checks whether the configured `plans_dir` (default `docs/plans`) is gitignored — some repos forbid committing tool or planning artifacts outright, which makes that path uncommittable — and falls back to `.drydock/plans/` when it is, alongside the other execution artifacts. Then **quote its `**Plan location:**` sentence verbatim** into the plan, directly under the title. Do not write your own justification for the location: that improvisation is the defect this replaces, because the reasoning came out different in every plan and a reader had to reconstruct why the file sat where it sat. One sentence, the same wording every time, generated rather than argued.
